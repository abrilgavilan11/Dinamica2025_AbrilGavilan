...
// --- 2. CARGAR LAS RUTAS DEFINIDAS ---
$routes = require BASE_PATH . '/config/routes.php';

// --- 3. BUSCAR Y MANEJAR LA RUTA ---
$routeFound = false;
...
function handleRoute($handler, $params = []) {
    ...
    if (class_exists($controllerClass)) {
        ...
        if (method_exists($controller, $method)) {
            // Llama al método del controlador, pasando los parámetros (ej. el ID del producto)
...
// Si no se encontró una ruta estática, buscamos rutas dinámicas (ej. /producto/{id}).
if (!$routeFound) {
    foreach ($routes[$requestMethod] ?? [] as $route => $handler) {
        ...
        if (!$routeFound) {
            // Convertir la ruta (ej. /producto/{id}) en una expresión regular
